...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, offset=(0, 0), tracks=None, fps=30, scale_factor=0.01):
    ...
    for i, box in enumerate(bbox):
        ...
        cv2.putText(img, label, (x1, y1 - 5),cv2.FONT_HERSHEY_SIMPLEX, 1.2, [255, 255, 255], 2)
        if tracks is not None:
            track = tracks[i]
            speed_kmh = compute_speed(track, fps, scale_factor)
            speed_text = f"{speed_kmh:.2f}km/h"
            cv2.putText(img, speed_text, (x2 + 5, y2), cv2.FONT_HERSHEY_SIMPLEX, 1.2, [0, 0, 255], 2)
...
class DetectionPredictor(BasePredictor):
    ...
    def write_results(self, idx, preds, batch):
        ...
        if len(tracked_dets)>0:
            ...
            draw_boxes(im0, bbox_xyxy, identities, categories, self.model.names, tracks=tracks, fps=YOUR_VIDEO_FPS, scale_factor=SCALE_FACTOR)
