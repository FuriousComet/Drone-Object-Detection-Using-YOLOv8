...
lines = {
    'horizontal_front': (2050, 550, 2250, 550),
    'horizontal_back': (1800, 1650, 2100, 1650),
    'vertical_left': (1300, 750, 1300, 1000),
    'vertical_right': (2500, 1100, 2500, 1400)
}
...
def check_line_crossing(track, line_name, line):
    ...
    if line_intersect(x1, y1, x2, y2, line_x1, line_y1, line_x2, line_y2):
        dy = y2 - y1  # Calculate vertical movement

        if line_name == 'horizontal_front' and dy < 0:  # Moving bottom to top
            return True
        elif line_name == 'horizontal_back' and dy > 0:  # Moving top to bottom
            return True
        elif line_name == 'vertical_left' and (x2 - x1) < 0:  # Moving right to left
            return True
        elif line_name == 'vertical_right' and (x2 - x1) > 0:  # Moving left to right
            return True
...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, offset=(0, 0), tracks=None, fps=60, scale_factor=0.01, lines=None, line_crossings=None):
    ...
    for i, box in enumerate(bbox):
        ...
        cv2.rectangle(img, (x1, y1 - 30), (x1 + w -70, y1), color, -1)
        ...
        text_background_color = (0, 255, 0)  # Green for text background
        text_color = (255, 255, 255)  # White for text
        text_margin = 1  # Margin around the text

        for line_name, line in lines.items():
            cv2.line(img, (line[0], line[1]), (line[2], line[3]), line_color, 8)
            midpoint = ((line[0] + line[2]) // 2, (line[1] + line[3]) // 2)
            text = f"{line_crossings[line_name]}"
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 4, 8)
            text_x = midpoint[0] - text_size[0] // 2
            text_y = midpoint[1] + text_size[1] // 2
            cv2.rectangle(img, (text_x - text_margin, text_y - text_size[1] - text_margin),
                        (text_x + text_size[0] + text_margin, text_y + text_margin), text_background_color, -1)
            cv2.putText(img, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 4, text_color, 8)
